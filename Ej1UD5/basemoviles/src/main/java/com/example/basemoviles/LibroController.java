...
public class LibroController {
  ...
  public ResponseEntity<Long> modificar(@PathVariable Long id, @RequestBody Map<String, String> res) throws URISyntaxException {
    Optional<Libro> libroExistente = libroRepositorio.findById(id);
    System.out.println(id);

    if (libroExistente.isPresent()) {
      Libro libroActual = libroExistente.get();
      // Update attributes of the existing Libro entity with the provided data
      libroActual.setNombre(res.get("nombre"));

      // Save the updated Libro entity
      Libro libroActualizado = libroRepositorio.save(libroActual);

      return ResponseEntity.status(HttpStatus.OK).body(libroActualizado.getIdLibro());
    } else {
      return ResponseEntity.notFound().build();
    }
  }
